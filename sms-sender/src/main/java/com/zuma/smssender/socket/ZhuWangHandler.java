...
public class ZhuWangHandler extends ChannelHandlerAdapter {
    ...
    @Autowired
    private ZhuWangSender zhuwangSender;

    //筑望同步(submit接口response)处理类
    @Autowired
    private ZhuWangSendSmsCallbackTemplate zhuWangSendSmsCallbackTemplate;

    //筑望异步(deliver接口request)处理类
    @Autowired
    private ZhuWangSendSmsAsyncCallbackTemplate zhuWangSendSmsAsyncCallbackTemplate;

    //筑望短信上行处理类
    @Autowired
    private ZhuWangSmsUpCallbackTemplate zhuWangSmsUpCallbackTemplate;
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        //给发送器设置上channel
        zhuwangSender.setChannel(ctx.channel());
