 ...
 * 筑望 发送短信
 */
public interface ZhuWangSubmitAPI {
    ...
    class Request extends ZhuWangHeader {
        ...
        public Request() {
            setCommandId(ZhuWangCommandIdEnum.CMPP_SUBMIT.getCode());
        ...
        public byte[] toByteArray() throws IOException {
            ...
            ZhuWangUtil.writeString(dous, this.serviceId, 10);//Service_Id 业务标识，是数字、字母和符号的组合。
            ...
            ZhuWangUtil.writeString(dous, this.feeTerminalId, 21);//Fee_terminal_Id 被计费用户的号码
            dous.writeByte(this.tpPid);//TP_pId
            dous.writeByte(this.tpUdhi);//TP_udhi
            dous.writeByte(this.msgFmt);//Msg_Fmt
            ZhuWangUtil.writeString(dous, this.msgSrc, 6);//Msg_src 信息内容来源(SP_Id)
            ZhuWangUtil.writeString(dous, this.feeType, 2);//FeeType 资费类别
            ZhuWangUtil.writeString(dous, this.feeCode, 6);//FeeCode
            ZhuWangUtil.writeString(dous, this.validTime, 17);//存活有效期
            ZhuWangUtil.writeString(dous, this.atTime, 17);//定时发送时间
            ZhuWangUtil.writeString(dous, this.srcId, 21);//Src_Id spCode
            dous.writeByte(this.destUsrTl);//DestUsr_tl
            ZhuWangUtil.writeString(dous, this.destTerminalId, 21);//Dest_terminal_Id
            dous.writeByte(this.msgLength);//Msg_Length
            dous.write(this.msgContent);//信息内容
            ZhuWangUtil.writeString(dous, this.reserve, 8);// 保留
    ...
    class Response extends ZhuWangHeader {
        ...
        public Response(byte[] data) throws IOException {
            ...
            ZhuWangSubmitErrorEnum resultEnum = EnumUtil.getByCode((int) this.getResult(), ZhuWangSubmitErrorEnum.class);
            this.setResultString(resultEnum == null ? ZhuWangSubmitErrorEnum.OTHER_ERROR.getMessage() : resultEnum.getMessage());
        }
    }
}
